...
public class MouseExample extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        Font bold = Font.font(last.getFont().getFamily(),
                                FontWeight.BOLD, 24);
        for(Node n : vbox.getChildren()) {
            if(n instanceof Label)
                ((Label)n).setFont(bold);
        }

        vbox.setOnMousePressed(e -> {
            last.setTextFill(Color.BLACK);
            last = labelPressed;
            labelPressed.setTextFill(Color.ORANGE);
        ...
        vbox.setOnMouseClicked(e -> {
            last.setTextFill(Color.BLACK);
            last = labelClicked;
            labelClicked.setTextFill(Color.CYAN);
            String s = new String("[" + (int) (e.getX()) + "," + (int) (e.getY()) + "]");
            labelCoords.setText(s);
        });
        ...
        vbox.setOnMouseEntered(e -> {
            last.setTextFill(Color.BLACK);
            last = labelEntered;
            labelEntered.setTextFill(Color.LIME);
            String s = new String("[" + (int) (e.getX()) + "," + (int) (e.getY()) + "]");
            labelCoords.setText(s);
        });
